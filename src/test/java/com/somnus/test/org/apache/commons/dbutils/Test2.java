...
public class Test2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (Syuser u : al) {
				List<Syrole> rl = qr.query(conn, "SELECT syrole.* FROM syrole JOIN syuser_syrole ON syuser_syrole.SYROLE_ID = syrole.ID WHERE syuser_syrole.SYUSER_ID = ?", new BeanListHandler<Syrole>(Syrole.class), u.getId());
