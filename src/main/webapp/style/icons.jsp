...
<script type="text/javascript">
	var selectIcon = function(window, input) {
		input.setValue($(':radio:checked').val());
		$(input.getEl().query('input')).removeClass(function(index,classes){
			var array = classes.split(' ');
			var result = [];
			$.each(array,function(index,item){
				if(Ext.String.startsWith(item, 'ext-icon')){
					result.push(item);
				}
			});
